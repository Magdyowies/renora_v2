...
export default function AdminDashboard() {
  ...
  const getRolePill = (role) => {
    ...
    return (
      <span
        className={`rounded-full px-3 py-1 text-xs font-semibold tracking-wider uppercase ${styles[role] || styles.customer}`}
      >
        {role}
      </span>
    );
          ...
          <div className="border-b border-neutral-200">
            <nav className="-mb-px flex space-x-6 px-6">
              <button
                ...
                className={`border-b-2 px-1 py-4 text-sm font-medium whitespace-nowrap ${activeTab === "overview" ? "border-primary text-primary" : "border-transparent text-neutral-500 hover:border-neutral-300 hover:text-neutral-700"}`}
              >
                Overview
              </button>
              <button
                onClick={() => setActiveTab("users")}
                className={`border-b-2 px-1 py-4 text-sm font-medium whitespace-nowrap ${activeTab === "users" ? "border-primary text-primary" : "border-transparent text-neutral-500 hover:border-neutral-300 hover:text-neutral-700"}`}
              >
                Users
              </button>
              <button
            ...
            {activeTab === "overview" && (
              <div className="grid gap-6 md:grid-cols-2">
                <div className="rounded-lg border border-neutral-200 p-5">
                  <h3 className="mb-4 flex items-center font-semibold text-neutral-800">
                    <TrendingUp className="mr-2 h-5 w-5" />
                    Recent Activity
                  </h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="text-neutral-600">Bookings Today</span>
                      <span className="font-medium">
                        {stats?.bookings_today || 0}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-neutral-600">Revenue Today</span>
                      <span className="font-medium">
